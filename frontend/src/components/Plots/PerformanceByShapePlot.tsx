...
export default function PerformanceByShapePlot() {
  const { token, user } = useAuth();

  const [stats, setStats] = useState<ILevel1Stat[]>([]);
  ...
  function generateData(): IData[] {
    ...
    for (const shape in barNames) {
      const shapeStats = stats.filter((stat) => {
        const shapeMatches = stat.expected_shape === shape;
        ...
        return shapeMatches && genderMatches && groupMatches && listNumMatches;
      });
      let correctStats = 0;

      for (let i = 0; i < shapeStats.length; i++) {
        if (shapeStats[i].is_correct) correctStats++;
      }

      let correctPercentage = (correctStats / shapeStats.length) * 100;
  ...
  return (
    <>
      <Box
        ...
        h={"100%"}
      >
        <Heading mb={5}>Rendimiento por tipo de figura</Heading>
        <Flex gap={2}>
          <FormControl>
