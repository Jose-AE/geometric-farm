...
import { ILevel2Stat } from "../../interfaces/level2stat.interface";
...
export default function PerformanceByOperationPlot() {
  const { token, user } = useAuth();

  const [stats, setStats] = useState<ILevel2Stat[]>([]);
  ...
  function generateData(): IData[] {
    ...
    for (const operation of barNames) {
      const operationStats = stats.filter((stat) => {
        const operationMatches = stat.operation_type === operation;
        ...
        return (
          operationMatches && genderMatches && groupMatches && listNumMatches
        );
      });
      let correctStats = 0;
  ...
  return (
    <>
      <Box
        ...
        h={"100%"}
      >
        <Heading mb={5}>Rendimiento por tipo de operación</Heading>
